...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <a href="<?= base_url('user/reject') ?>" type="button" class="btn bg-orange btn-lg"><i class="fa fa-clipboard"></i> Histori jasa tidak bisa scrap</a>
    ...
    <section class="content-header">
        ...
        <div class="callout callout-success">
            ...
            <p>! Input hanya JASA SAJA</p>
            <p>! Data jasa/MATERIAL di Inputkan di Harga MATERIAL</p>
        </div>
        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="box">
                    ...
                    <!-- /.box-header -->
                    <div class="box-body">
                        <table id="tableJasa" class="table table-bordered table-hover">
                            <thead>
                                <tr>
                                    <th style="width: 5%">NO</th>
                                    <th style="width: 60%">Kode jasa</th>
                                    <th>Nama jasa</th>
                                    <th>Register Date</th>
                                    <th>AKSI</th>
                                </tr>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Ubah jasa</h4><br>
            </div>
            <form id="form_update_jasa" method="post" enctype="multipart/form-data">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">TAMBAH JASA</h4><br>
            </div>
...
<script>
    ...
    $(function() {
        $("#tableJasa").DataTable({
            ...
            'ajax': {
                'dataType': 'json',
                'url': '<?= base_url('user/ajax_table_jasa') ?>',
                'type': 'post',
            },
            'columns': [{
                "target": [<?= $target ?>],
                ...
                "render": function(data) {
                    return `<button class="btn btn-sm btn-danger" onclick="delete_data('` + data.id + `')"><i class="fa fa-trash"></i> Hapus</button>&nbsp;<button class="btn btn-sm btn-warning" onclick="ubah_data('` + data.id + `','` + data.nama_jasa + `')"><i class="fa fa-edit"></i> Ubah</button>`
                }
            }, ],
    ...
    function reload_table() {
        $('#tableJasa').DataTable().ajax.reload(null, false);
    }
    ...
    $("#form_update_jasa").submit(function(e) {
        e.preventDefault()

        if ($('#id_jasa').val() == '' || $('#edit_nama_jasa').val() == '') {
            Swal.fire(
                'error!',
